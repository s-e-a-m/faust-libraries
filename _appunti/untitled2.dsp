...
vcs3osc1(freq,shape,slev,plev) = ((wsin+sine)*slev),(saw*plev)
  with{
    step = freq/ma.SR;
    phasor(step) = step : (+ : ma.decimal)~_ : -(step);
    sine = sin(phasor(step)*2*ma.PI) : *(0.5*sin(shape*(ma.PI)));
    wsin = sin(phasor(step)*(-1)*ma.PI) : +(0.5) : *(cos(shape*(ma.PI)));
    pshape = cos(shape*(ma.PI))*(-1);
    saw = phasor(step)-(0.5);
};
//process = vcs3osc1(freq,shape,slev,plev);


phasor1(step) = step : (+ : ma.decimal)~_;
phasor2(step) = +(step)~ ma.decimal : -(step);
process = phasor1, phasor2;
